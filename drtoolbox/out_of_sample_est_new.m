function t_points = out_of_sample_est_new(points, X, mappedX, nn)
...
%         [d, ind] = min(d);
        ...
        L = pinv(X(idx(1),:) - mean(X(idx(1:nn),:))) * (mappedX(idx(1),:) - mean(mappedX(idx(1:nn),:)));

        % Compute coordinates of transformed point
        t_points(i,:) = (mean(mappedX(idx(1:nn),:)) + ((point - mean(X(idx(1:nn),:))) * L))';
    end
